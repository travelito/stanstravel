...
export async function generateMetadata({
  ...
  return {
    title: `${t(locale, "trainTicketsTitle")} — ${t(locale, "siteName")}`,
    alternates: localizedAlternates("/train-tickets", locale),
  };
}
    ...
    <div className="mx-auto max-w-7xl px-6 py-16">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {tickets.map((tt) => (
          <Link
            key={tt.slug}
            href={`/${locale}/train-tickets/${tt.slug}`}
          ...
          >
            <p className="font-mono text-xs text-turquoise uppercase tracking-wide">
              {tt.fromCity} → {tt.toCity} · {tt.train}
            </p>
            <h2 className="font-display text-lg mt-1">{tt.title[locale]}</h2>
            <p className="text-sm text-ink/70 mt-2">{tt.summary[locale]}</p>
            <div className="flex justify-between mt-4 font-mono text-sm">
              <span>
                {tt.durationHours} {t(locale, "hours")}
              </span>
              <span className="text-indigo font-semibold">
                {t(locale, "fromPrice")} ${tt.priceUsd}
              </span>
            </div>
          </Link>
        ))}
