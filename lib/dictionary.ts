import type { Locale } from "@/lib/locales";

export const dictionary: Record<Locale, Record<string, string>> = {
  ru: {
    siteName: "StansTravel",
    tagline: "Туры и экскурсии по Узбекистану",
    navTours: "Туры",
    navExcursions: "Экскурсии",
    navTransfers: "Трансферы",
    navTrainTickets: "Жд-билеты",
    navAbout: "О нас",
    navHome: "Главная",
    heroTitle: "Дорога, которой шли караваны",
    heroSubtitle:
      "Однодневные туры и экскурсии по Самарканду, Бухаре и Хиве — с гидом, без спешки.",
    heroCta: "Смотреть туры",
    toursTitle: "Туры по городам",
    excursionsTitle: "Короткие экскурсии",
    transfersTitle: "Трансферы между городами",
    trainTicketsTitle: "Билеты на скоростные поезда",
    aboutTitle: "Почему выбрать нас",
    aboutIntro:
      "Мы — туристическая компания с опытом работы более пятнадцати лет в организации туров по Узбекистану и Центральной Азии.",
    aboutReason1Title: "Пятнадцать лет опыта",
    aboutReason1Body:
      "Мы организуем туры и экскурсии по Узбекистану уже больше пятнадцати лет и знаем регион изнутри.",
    aboutReason2Title: "Личный подход",
    aboutReason2Body:
      "Каждый маршрут можно адаптировать под ваши пожелания — по времени, темпу и интересам.",
    aboutReason3Title: "Надёжная организация",
    aboutReason3Body:
      "Билеты, трансферы и гиды бронируются заранее — вы получаете подтверждение до начала поездки.",
    contactsTitle: "Контакты",
    contactsPhone: "Телефон",
    contactsWhatsapp: "Написать в WhatsApp",
    quickContactWhatsapp: "Написать в WhatsApp",
    quickContactTelegram: "Написать в Telegram",
    quickContactCall: "Позвонить",
    quickContactMessage: "Здравствуйте! Интересует тур на StansTravel",
    share: "Поделиться",
    shareWhatsapp: "Отправить в WhatsApp",
    shareTelegram: "Отправить в Telegram",
    shareCopyLink: "Скопировать ссылку",
    shareCopied: "Скопировано",
    fromPrice: "от",
    perPerson: "с человека",
    duration: "Длительность",
    hours: "ч",
    viewDetails: "Подробнее",
    backToTours: "Все туры",
    backToExcursions: "Все экскурсии",
    bookingDate: "Дата",
    bookingStartTime: "Время начала",
    bookingTravelers: "Путешественники",
    bookingPickupLocation: "Место встречи",
    bookingPickupPlaceholder: "Отель, адрес, аэропорт…",
    bookingDatePlaceholder: "Выберите дату",
    bookingTimePlaceholder: "Выберите время",
    bookingPaymentNote: "💳 Оплата возможна в российских рублях.",
    bookingNotSpecified: "не указано",
    bookingTotalPrice: "Итого",
    bookingGreeting: "Здравствуйте!",
    bookingIntro: "Хочу забронировать эту услугу.",
    bookingLabelTour: "Тур",
    bookingLabelExcursion: "Экскурсия",
    bookingClosing: "Пожалуйста, подтвердите наличие мест.",
    priceContactMessage: "Свяжитесь с нами для расчёта стоимости",
    priceContactIntro: "Хочу узнать стоимость для такого количества человек.",
    tierTableTravelers: "Чел.",
    tierTableTotal: "Всего",
    tierTablePerPerson: "За чел.",
    aboutThisTour: "Об экскурсии",
    itineraryTitle: "Маршрут",
    includedTitle: "Включено",
    notIncludedTitle: "Не включено",
    quickInfoPrivateTour: "Индивидуальный тур",
    quickInfoGroupTour: "Групповой тур",
    quickInfoPickupIncluded: "Трансфер включён",
    quickInfoPaceWalking: "Пешая экскурсия",
    quickInfoPaceTransport: "На транспорте",
    quickInfoPaceMixed: "Пешком + транспорт",
    highlights: "Что вы увидите",
    photoOf: "из",
    seeAllExcursionsIn: "Смотреть все экскурсии в",
    filteredByCity: "Город",
    showAll: "Показать все",
    excursionsAllCities: "Все города",
    footerRights: "Все права защищены.",
    notFoundTitle: "Страница не найдена",
    notFoundBody: "Такой страницы не существует — возможно, она была перемещена или удалена.",
    notFoundCta: "Вернуться на главную",
  },
  en: {
    siteName: "StansTravel",
    tagline: "Tours and Excursions across Uzbekistan",
    navTours: "Tours",
    navExcursions: "Excursions",
    navTransfers: "Transfers",
    navTrainTickets: "Train Tickets",
    navAbout: "About",
    navHome: "Home",
    heroTitle: "The road the caravans walked",
    heroSubtitle:
      "Day tours and excursions in Samarkand, Bukhara, and Khiva — guided, unhurried.",
    heroCta: "Browse tours",
    toursTitle: "Tours by City",
    excursionsTitle: "Short Excursions",
    transfersTitle: "Transfers between Cities",
    trainTicketsTitle: "High-Speed Train Tickets",
    aboutTitle: "Why choose us",
    aboutIntro:
      "We're a tourism company with over fifteen years of experience organizing tours across Uzbekistan and Central Asia.",
    aboutReason1Title: "Fifteen years of experience",
    aboutReason1Body:
      "We've been organizing tours and excursions across Uzbekistan for more than fifteen years and know the region inside out.",
    aboutReason2Title: "A personal approach",
    aboutReason2Body:
      "Every route can be adapted to your preferences — timing, pace, and interests.",
    aboutReason3Title: "Reliable organization",
    aboutReason3Body:
      "Tickets, transfers, and guides are booked in advance — you get confirmation before the trip starts.",
    contactsTitle: "Contacts",
    contactsPhone: "Phone",
    contactsWhatsapp: "Message on WhatsApp",
    quickContactWhatsapp: "Message on WhatsApp",
    quickContactTelegram: "Message on Telegram",
    quickContactCall: "Call us",
    quickContactMessage: "Hi! I'm interested in a tour on StansTravel",
    share: "Share",
    shareWhatsapp: "Share on WhatsApp",
    shareTelegram: "Share on Telegram",
    shareCopyLink: "Copy link",
    shareCopied: "Copied",
    fromPrice: "from",
    perPerson: "per person",
    duration: "Duration",
    hours: "h",
    viewDetails: "View details",
    backToTours: "All tours",
    backToExcursions: "All excursions",
    bookingDate: "Date",
    bookingStartTime: "Start time",
    bookingTravelers: "Travelers",
    bookingPickupLocation: "Pickup location",
    bookingPickupPlaceholder: "Hotel, address, airport…",
    bookingDatePlaceholder: "Select date",
    bookingTimePlaceholder: "Select time",
    bookingPaymentNote: "💳 Payment in Russian rubles is available.",
    bookingNotSpecified: "not specified",
    bookingTotalPrice: "Total Price",
    bookingGreeting: "Hello!",
    bookingIntro: "I want to request this tour.",
    bookingLabelTour: "Tour",
    bookingLabelExcursion: "Excursion",
    bookingClosing: "Please confirm availability.",
    priceContactMessage: "Please contact us for this group size",
    priceContactIntro: "I'd like to know the price for this many travelers.",
    tierTableTravelers: "Travelers",
    tierTableTotal: "Total",
    tierTablePerPerson: "Per person",
    aboutThisTour: "About this tour",
    itineraryTitle: "Itinerary",
    includedTitle: "What's included",
    notIncludedTitle: "What's not included",
    quickInfoPrivateTour: "Private Tour",
    quickInfoGroupTour: "Group Tour",
    quickInfoPickupIncluded: "Hotel Pickup",
    quickInfoPaceWalking: "Walking Tour",
    quickInfoPaceTransport: "By Transport",
    quickInfoPaceMixed: "Walking + Transport",
    highlights: "What you'll see",
    photoOf: "of",
    seeAllExcursionsIn: "See all excursions in",
    filteredByCity: "City",
    showAll: "Show all",
    excursionsAllCities: "All cities",
    footerRights: "All rights reserved.",
    notFoundTitle: "Page not found",
    notFoundBody: "This page doesn't exist — it may have been moved or removed.",
    notFoundCta: "Back to homepage",
  },
};

export function t(locale: Locale, key: string): string {
  return dictionary[locale][key] ?? key;
}
